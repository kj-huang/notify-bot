...
model.pushMarketingMsgTo = async function (ID) {
  await LineBot.pushText(ID, '活動組 佳佳：提醒妳今天需在各大群組發佈下週四的讀書會主題。Robot 溫馨提醒');
}

model.pushMsgTo = async function (ID) {
  await LineBot.pushText(ID, '活動組 佳佳：提醒你今天需在主群組接龍開始報名這次的活動，記得放活動DM。Robot 溫馨提醒');
}

model.pushActMsgTo = async function (ID) {
  await LineBot.pushText(ID, '講師組 Lena：提醒你今天需上傳主群組接龍需放講員講義的連結。Robot 溫馨提醒');
}


model.pushActivityMsgTo = async function (ID) {
  await LineBot.pushText(ID, '主持人：提醒您今天讀書會，上午1200前要發三大社群(不包含個案社)的今晚的活動預告，要加上講員講義。主群是接龍PO文+講員講義再度提醒。Robot 溫馨提醒');
}

model.pushRetroMsgTo = async function (ID) {
  await LineBot.pushText(ID, '資訊組 世嵩：提醒今天要提供給五大群組這次分享的影片及下次活動DM。Robot 溫馨提醒');
}
