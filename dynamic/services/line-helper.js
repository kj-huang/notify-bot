...
model.pushActMsgTo = async function (ID) {
  await LineBot.pushText(ID, '講師組 Lena：提醒你今天需上傳主群組接龍需放講員講義的連結，並且將講義連結放置PO文接龍表。Robot 溫馨提醒');
}

model.pushActMsg2To = async function (ID) {
  await LineBot.pushText(ID, '主持人 Roger：提醒你今天需請主講人1.觀看主講人SOP影片 2.請講師當天2050上線測試 3.開始前要求與會者寫FB100字心得');
}
...
model.pushActivityMsg3To = async function (ID) {
  await LineBot.pushText(ID, 'FB小編 京德:提醒您今天1200要開100字的FB心得分享區，並放在大群給有參與讀書會的人填寫。Robot 溫馨提醒');
}
...
model.pushRetroMsgTo = async function (ID) {
  await LineBot.pushText(ID, '資訊組 玉嬌：提醒今天要提供給三大社群這次分享的FB影片及下次活動DM。主群則提供影片連結，Robot 溫馨提醒');
}
