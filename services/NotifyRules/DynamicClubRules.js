...
class AuditMessage {
    ...
    CheckNotifyDate(scheduledDate) {
        ...
        if (date === this.today) {
            return '提醒會員組虹秋開始準備稽核，第一次的稽核是稽核是否有填會員表連續五次出席讀書會，及有報名未出席讀書會，稽核對象為動態競爭相關社群';
        }
        return '';
    }
}
...
class ActivityMessage {
    ...
    CheckNotifyDate(scheduledDate) {
        ...
        if (date === this.today) {
            return 'Robot提醒1.講師組 Lena：提醒你今天需上傳主群組接龍需放講員講義的連結，並且將講義連結放置PO文接龍表。\nRobot提醒2.主持人 Roger：提醒你今天需請主講人1.觀看主講人SOP影片 2.請講師當天2050上線測試 3.開始前要求與會者寫FB100字心得。';
        }
        return '';
    }
}
...
class MeetingFBMessage {
    ...
    CheckNotifyDate(scheduledDate) {
        let date = moment(scheduledDate).tz("Asia/Taipei").format("YYYYMMDD");
        if (date === this.today) {
            return 'FB小編 京德:提醒您今天1200要開100字的FB心得分享區，並放在大群給有參與讀書會的人填寫。Robot 溫馨提醒';
        }
        return '';
    }
}
...
class RetroMessage {
    ...
    CheckNotifyDate(scheduledDate) {
        ...
        if (date === this.today) {
            return '資訊組 玉嬌：提醒今天要提供給三大社群這次分享的FB影片及下次活動DM。主群則提供影片連結，Robot 溫馨提醒';
        }
        return '';
    }
}
...
class PostAuditMessage {
    ...
    CheckNotifyDate(scheduledDate) {
        ...
        if (date === this.today) {
            return '提醒會員組虹秋稽核是否有加入大大社群，稽核對象為動態競爭相關社群';
        }
        return '';
    }
}
