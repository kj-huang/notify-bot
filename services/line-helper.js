...
model.pushAuditMsgTo = async function (ID) {
  await LineBot.pushText(ID, '提醒會員組虹秋開始準備稽核，第一次的稽核是稽核是否有填會員表連續五次出席讀書會，及有報名未出席讀書會，稽核對象為跑統計學術群、實務群及大大社群');
}
...
model.pushRetroAuditMsgTo = async function (ID) {
  await LineBot.pushText(ID, '提醒會員組虹秋稽核是否有加入大大社群，稽核對象為跑統計學術群、實務群及大大社群+偉豪學院及Python社群');
}
